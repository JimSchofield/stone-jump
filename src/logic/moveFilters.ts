        ...
        return board.grid.map(row => {
            return row.map(cell => {
                if (cell.value === "1") {
                    ...
                    const { x: toToX, y: toToY } = geometry[moveName](toX,toY);
                    ...
                    const nextNextCell = Maybe.from(board)
                        ...
                        .join() as unknown as string;
                    if (nextCell === "1" && nextNextCell === "0") {
                        return new Move({ x: cell.x, y: cell.y }, {x: toToX, y: toToY});
                    }
                }
            })
        })
...
export const assembleCartesianFromSelected = assembleMovesFromSelected(cartesian);
